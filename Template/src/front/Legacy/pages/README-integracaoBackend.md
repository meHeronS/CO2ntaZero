...
-   **Registrar um novo consumo:** `POST /api/consumptions`
    -   **Body**: `{ "resourceType": "electricity", "quantity": 500, "unit": "kWh", "date": "2025-11-13T12:00:00.000Z" }`
...
-   **Criar uma nova meta:** `POST /api/goals`
    -   **Body**: `{ "title": "Reduzir consumo de energia", "targetReductionPercentage": 15, "resourceType": "electricity" }`
...
-   **Exportar Relatório de Emissões em PDF:** `GET /api/reports/export/emissions-pdf`
    ...
    -   **Exemplo de implementação no Frontend:**
        ...
        fetch('/api/reports/export/emissions-pdf', {
            method: 'GET',
...
O backend agora gera alertas automaticamente quando as metas ambientais e os limites de consumo são atingidos. O frontend precisa buscar e exibir esses alertas para o usuário.
