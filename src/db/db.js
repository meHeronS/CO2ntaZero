  ...
  try {
    ...
    for (const collectionName of expectedCollections) {
      if (!existingCollections.includes(collectionName)) {
        console.log(`[DB Setup] 🛠️  Criando collection ausente: '${collectionName}'...`);
    ...
    console.error('⚠️ Aviso durante a validação de collections:', error.message);
  }
};
...
const checkSystemHealth = () => {
  ...
  if (!process.env.MONGO_URI) {
    ...
    console.error('➡️  O sistema não conseguirá conectar ao banco. Verifique seu arquivo .env ou o Docker Compose.');
    process.exit(1);
  }
